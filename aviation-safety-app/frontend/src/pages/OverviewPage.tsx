...
import { Section, StatGrid, MetricCard, Card, CardHeader, Badge, Table, ProgressBar } from '@/components/ui/DataDisplay';
import { BarChartComponent, PieChartComponent } from '@/components/charts/ChartComponents';
...
export function OverviewPage() {
  ...
  const imbalanceRatio = Object.values(data.class_distribution).length > 1 
    ? Math.max(...Object.values(data.class_distribution)) / Math.min(...Object.values(data.class_distribution))
    : 1;
    ...
    <div className="space-y-6">
      ...
      <StatGrid columns={4}>
        ...
        <MetricCard
          ...
          changeType={imbalanceRatio > 100 ? 'negative' : imbalanceRatio > 10 ? 'warning' : 'positive'}
        />
      </StatGrid>
          ...
          <div className="flex flex-wrap gap-2 mt-4">
            {Object.entries(data.domains).map(([domain, count]) => (
              <Badge key={domain} variant="info" size="sm">
                {domain}: {formatNumber(count)} ({(count/total*100).toFixed(1)}%)
              </Badge>
            ))}
          </div>
        </Card>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <Card className="bg-blue-50 border-blue-200">
          ...
          <div className="space-y-2">
            {data.narrative_stats && Object.entries(data.narrative_stats).map(([key, value]) => (
              <div key={key} className="flex justify-between">
                <span className="text-brown-600 capitalize">{key.replace('_', ' ')}</span>
                <span className="font-medium text-brown-800">{typeof value === 'number' ? value.toFixed(0) : value}</span>
              </div>
            ))}
          </div>
        </Card>
