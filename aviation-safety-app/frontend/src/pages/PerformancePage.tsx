...
import { Section, Card, CardHeader, MetricCard, Table, ProgressBar, StatGrid } from '@/components/ui';
import { BarChartComponent } from '@/components/charts/ChartComponents';
import { Loader2, CheckCircle, AlertCircle, FileText, Award } from 'lucide-react';
...
export function PerformancePage() {
  ...
  if (error || !data) {
    return (
      <Card className="text-center py-12">
        ...
        <p className="text-brown-500 mb-4">{error || 'No performance data available. Run pipeline first.'}</p>
  ...
  const classDistData = classification_report.map((row: { class_name: string; precision: number; recall: number; f1_score: number; support: number }) => ({
  ...
  const supportData = classification_report
    ...
    .slice(0, 10)
    .map((row: { class_name: string; support: number }) => ({
      name: row.class_name.length > 20 ? row.class_name.slice(0, 20) + '...' : row.class_name,
      support: row.support,
    }));
          ...
          <Card>
            ...
            <div className="h-80">
              <BarChartComponent
                ...
                barColors={['#5C4033', '#8B6B4D', '#0F766E']}
                horizontal={true}
                height={350}
              />
            </div>
        ...
        {metrics.per_class_metrics && (
          <Card>
            ...
            <Table
              ...
              data={Object.entries(metrics.per_class_metrics as Record<string, Record<string, number>>).map(([className, vals]: [string, Record<string, number>]) => ({
                class: className,
                ...vals,
              }))}
